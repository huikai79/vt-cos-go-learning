...
> 當前補遺：跨課銜接修正後，介面版本為 `learner-flow-v27`、儲存 schema 為 7、內容目錄版本為 3。完成同課題目時直接前往下一題；跨課或跨單元時，按鈕明示將進入短講，並自動聚焦新課標題、短講及棋盤示範，流水線回到「先看懂」。短講待看狀態會跨重新載入保存。19 課現在都有至少兩步棋盤示範；第 9–19 課的 5×5 縮圖明示為局部比較或階段示意，不冒充唯一全局答案。10 個後續單元另有 9×9 的局部觀察點選；它只判定題幹指定要點，不能用來判定全局最佳手。首頁只在有題目真正到期時顯示「今日到期」與數量；R1a 已從學習者工具選單移除，審查頁明示只供不同於學習者的外部審查者使用。七天批次為 `personal-pilot-v3`，八題全來自舊 R1 自我審查中已曝光的 22 題，只檢查操作、返回、資料與負擔。審查母體仍為 77 題；R1b 難度可比性維持未知。下段 `learner-flow-v20`／`v23`／`v24`／`v25`／`v26` 為歷史快照，與本補遺衝突時以本補遺及 `COMPLETION_MATRIX.md` 為準。
...
| 項目 | 結果 | 證據或限制 |
|---|---|---|
| 桌面閱讀性 | 通過 | 內文、題幹為 16px，主標為 32px；棋盤與題幹並排，留白足夠。 |
