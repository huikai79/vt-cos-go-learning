...
test("間隔練習保存選題政策與作答後的下一次到期時間", async () => {
  ...
  assert.equal(exported.uiVersion, "learner-flow-v42");
});
...
test("個人 pilot 禁用提示、只收首答，而且不污染課程進度與錯題", async () => {
  ...
  assert.equal(saved.trial.answers[0].uiVersion, "learner-flow-v42");
...
test("CJK learner UI 使用繁中語系、適當字型 fallback 與安全換行策略", () => {
  ...
  assert.match(html, /styles\.css\?v=learner-flow-v42/);
