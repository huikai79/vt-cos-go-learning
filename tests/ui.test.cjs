...
async function main() {
  ...
  try {
    ...
    assert.ok(captureEvents.every((event) => event.uiVersion === "learner-flow-v40"));
    ...
    assert.equal(rawEvents.uiVersion, "learner-flow-v40");
    ...
    assert.equal(rawEvents.applicationResults[0].uiVersion, "learner-flow-v40");
    assert.equal(rawEvents.trial.answers.length, 1);
    assert.equal(rawEvents.trial.answers[0].uiVersion, "learner-flow-v40");
