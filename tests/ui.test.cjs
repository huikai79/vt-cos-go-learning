...
async function main() {
  ...
  try {
    ...
    assert.deepEqual(courseShape, { units: 15, shownUnits: 1, lessons: 3, toolDescriptions: 9, advancedTrainingLink: "進階訓練", r1LinkAbsent: true, contextBars: 1, advancedClosed: true, advancedLabel: "進階設定與資料 通常不需要現在處理", rawBackupHint: "下載可重算的 JSON 原始事件與局部復盤資料；請自行妥善保存，不需要每天匯出。" });
