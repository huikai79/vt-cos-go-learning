...
test("棋盤練習的操作提示與即時回饋維持 16px 級核心可讀性", () => {
  const css = fs.readFileSync(path.join(__dirname, "..", "live-game.css"), "utf8");
  assert.match(css, /\.live-board-help\{[^}]*font-size:1rem;[^}]*line-height:1\.6/);
  assert.match(css, /\.live-feedback\{[^}]*font-size:1rem;[^}]*line-height:1\.6/);
